...
namespace KatlaSport.WebApi.Controllers
{
    ...
    public class HiveSectionsController : ApiController
    {
        ...
        public async Task<IHttpActionResult> AddHiveSection([FromBody] UpdateHiveSectionRequest createRequest)
        {
            ...
            var haveSection = await _hiveSectionService.CreateHiveSectionAsync(createRequest);//_hiveService.CreateHiveAsync(createRequest);
            var location = $"/api/sections/{haveSection.Id}";
            return Created<HiveSection>(location, haveSection);
        }
        ...
        public async Task<IHttpActionResult> DeleteHiveSection([FromUri] int hiveSectionId)
        {
            if (hiveSectionId < 1)
            {
                return BadRequest($"Invalid {nameof(hiveSectionId)}");
            }
